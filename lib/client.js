...
Client.prototype.connect = function () {
    ...
    zk.once('init', function (broker) {
        ...
        for (b in broker) {
            addr = broker[b].host + ':' + broker[b].port;
...
Client.prototype.loadMetadataForTopics = function (topics, cb) {
    var correlationId = this.nextId(),
        request = protocol.encodeMetadataRequest(this.clientId, correlationId, topics),
        broker = this.brokerForLeader();
...
Client.prototype.createTopics = function (topics, cb) {
    ...
    topics.forEach(function (topic) {
        this.zk.topicExists(topic, function (existed, reply) {
            ...
            if (existed && created === topics.length) {
...
Client.prototype.nextId = function () {
    var currentId = this.correlationId;
...
Client.prototype.nextPartition = (
    function cycle () {
        var c = 0;

        return function (topic) {
            var index = c % this.topicPartitions[topic].length;
            c += 1;

            return this.topicPartitions[topic][index];
        }
    }()
);
...
Client.prototype.sendToBroker = function (payloads, encoder, decoder, cb) {
    ...
    for (leader in payloads) {

        correlationId = this.nextId();

        request = encoder.call(
            null,
            this.clientId,
            correlationId,
            payloads[leader]
        ...
        broker = this.brokerForLeader(leader);

        if (broker.error) {
            return cb('Leader not available', payloads[leader]);
        }
...
Client.prototype.hasMetadata = function (topic, partition) {
    var brokerMetadata = this.brokerMetadata,
        ...
        leader = topicMetadata[topic] &&
            topicMetadata[topic][partition] &&
            topicMetadata[topic][partition].leader;
...
Client.prototype.updateMetadatas = function (metadatas) {
    ...
    for (topic in this.topicMetadata) {
...
Client.prototype.removeTopicMetadata = function (topics, cb) {
    topics.forEach(function (t) {
        if (this.topicMetadata[t]) {
            delete this.topicMetadata[t];
        }
    }.bind(this));
...
Client.prototype.brokerForLeader = function (leader) {
    ...
    // If leader is not given, choose the first broker as leader
    if (leader === undefined) {
    ...
    metadata = this.brokerMetadata[leader];
...
Client.prototype.handleReceivedData = function (socket) {
    ...
    if (socket.buffer.length >= size) {
        var resp     = socket.buffer.slice(0, size),
            handlers = this.cbqueue[correlationId],
            decoder  = handlers[0],
            cb       = handlers[1];
